...
class FetchTransactionInformationHandler implements HandlerInterface
{
    ...
    private function saveHostedFieldsCustomerCard(
        ...
        $hfIdentifier = $this->payplugConfig->getHostedFieldsIdentifier($websiteId);
        ...
        $tokenDetails = [
            PaymentTokenInterface::DETAIL_BRAND => strtolower((string) ($paymentResource->card->brand ?? '')),
        ...
        try {
            $fullYear = strlen((string) $expYear) === 4 ? $expYear : 2000 + $expYear;
            $expiryDateTime = new DateTime(sprintf('%04d-%02d-01 00:00:00', $fullYear, $expMonth));
        } catch (Throwable) {
